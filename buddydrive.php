...
$MyUpdateChecker = Puc_v4_Factory::buildUpdateChecker(
	'https://n3rds.work//wp-update-server/?action=get_metadata&slug=psourcedrive', 
	__FILE__, 
	'psourcedrive' 
);
...
class BuddyDrive {
	...
	public function warning() {
		...
		if ( ! self::version_check() ) {
			$warnings[] = sprintf( esc_html__( 'PS Buddy Drive erfordert mindestens Version %s von BuddyPress.', 'buddydrive' ), self::$required_bp_version );
			$resolve[]  = sprintf( esc_html__( 'Aktualisiere BuddyPress auf mindestens Version %s', 'buddydrive' ), self::$required_bp_version );
		}
		...
		if ( ! $config['blog_status'] ) {
			$warnings[] = esc_html__( 'PS Buddy Drive muss in dem Blog aktiviert werden, in dem BuddyPress aktiviert ist.', 'buddydrive' );
			$resolve[]  = esc_html__( 'Aktiviere PS Buddy Drive im selben Blog wie BuddyPress', 'buddydrive' );
		}

		if ( ! $config['network_status'] ) {
			$warnings[] = esc_html__( 'PS Buddy Drive und BuddyPress müssen dieselbe Netzwerkkonfiguration verwenden.', 'buddydrive' );
			$resolve[]  = esc_html__( 'Stelle sicher, dass PS Buddy Drive auf der gleichen Ebene wie BuddyPress im Netzwerk aktiviert ist', 'buddydrive' );
		}
		...
		if ( ! empty( $warnings ) ) {
			...
			if ( is_super_admin() ) {
				...
				$deactivate_link = '<a href="' . esc_url( $deactivate_link ) . '">' . esc_html__( 'deaktivieren', 'buddydrive' ) . '</a>';
				$resolve_message = '<ol><li>' . sprintf( __( 'Du solltest %s PS Buddy Drive', 'buddydrive' ), $deactivate_link ) . '</li>';
				...
				if ( $config['network_status'] && $config['blog_status']  )
					$resolve_message .= '<li>' . esc_html__( 'Wenn Du fertig bist, versuche erneut, PS Buddy Drive zu aktivieren.', 'buddydrive' ) . '</li></ol>';
