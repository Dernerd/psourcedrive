...
function buddydrive_admin_get_settings_sections() {
	return (array) apply_filters( 'buddydrive_admin_get_settings_sections', array(
		'buddydrive_settings_main' => array(
			'title'    => __( 'Haupteinstellungen', 'buddydrive' ),
			'callback' => 'buddydrive_admin_setting_callback_main_section',
			'page'     => 'buddydrive',
		),
		'buddydrive_settings_customs' => array(
			'title'    => __( 'Benutzerdefinierte Slug- und Namenseinstellungen', 'buddydrive' ),
	...
	return (array) apply_filters( 'buddydrive_admin_get_settings_fields', array(
		...
		'buddydrive_settings_main' => array(
			...
			'_buddydrive_default_privacy' => array(
				'title'             => __( 'Standard-Datenschutz für neue Elemente', 'buddydrive' ),
			...
			'_buddydrive_user_quota' => array(
				'title'             => __( 'Verfügbarer Speicherplatz für jeden Benutzer', 'buddydrive' ),
			...
			'_buddydrive_max_upload' => array(
				'title'             => __( 'Maximale Upload-Größe', 'buddydrive' ),
			...
			'_buddydrive_allowed_extensions' => array(
				'title'             => __( 'Mime-Typen erlaubt', 'buddydrive' ),
			...
			'_buddydrive_auto_group' => array(
				'title'             => __( 'BuddyDrive für Gruppen bei der Gruppenerstellung aktivieren', 'buddydrive' ),
		...
		'buddydrive_settings_customs' => array(
			...
			'_buddydrive_user_subnav_name' => array(
				'title'             => __( 'Name für Hauptsubnav', 'buddydrive' ),
			...
			'_buddydrive_friends_subnav_slug' => array(
				'title'             => __( 'Slug für Freunde Subnav', 'buddydrive' ),
			...
			'_buddydrive_friends_subnav_name' => array(
				'title'             => __( 'Name für Freunde Subnav', 'buddydrive' ),
...
?>
	...
	<p><?php _e( 'Passe Deinen PS Buddy Drive an!', 'buddydrive' ); ?></p>
...
?>
	...
	<p><?php _e( 'Passe die Slugs und Namen von PS Buddy Drive an!', 'buddydrive' ); ?></p>

	<?php if( !empty( $page_id ) ) :?>
		<p class="description"><?php printf( __( 'Hinweis: Um den Namen und den Slug der Hauptverzeichnisseite anzupassen, musst Du den Titel und den Permalink der <a href="%s">WordPress-Seite</a> bearbeiten', 'buddydrive' ), esc_url( get_edit_post_link( $page_id ) ) );?></p>
		...
		<?php foreach ( $roles as $role => $details ) :
			...
			<tr>
				...
				<td>
					...
					<label for="_buddydrive_user_quota[<?php echo $role;?>]"><?php _e( 'MB', 'buddydrive' ); ?></label>
				</td>
			</tr>
...
function buddydrive_admin_setting_callback_max_upload() {
	...
	<label for="_buddydrive_max_upload"><?php _e( 'MB', 'buddydrive' ); ?></label>
	<?php
}
...
function buddydrive_admin_setting_callback_allowed_extensions() {
	...
	?>
	<ul>
		<li><input type="checkbox" id="buddydrive-toggle-all" checked /> <?php _e( 'Alle auswählen/abwählen', 'buddydrive' );?></li>
...
function buddydrive_admin_setting_callback_user_subnav_name() {
	$user_subnav = bp_get_option( '_buddydrive_user_subnav_name', __( 'PS Buddy Drive Dateien', 'buddydrive' ) );
...
function buddydrive_admin_setting_callback_friends_subnav_name() {
	$friends_subnav = bp_get_option( '_buddydrive_friends_subnav_name', __( 'Zwischen Freunden', 'buddydrive' ) );
...
?>
	...
	<div class="wrap">
		...
		<h2><?php _e( 'PS Buddy Drive Einstellungen', 'buddydrive' ) ?></h2>
		...
		<form action="<?php echo esc_url( $form_action );?>" method="post">
			...
			<p class="submit">
				...
				<?php endif;?>
				<input type="submit" name="submit" class="button-primary" value="<?php esc_attr_e( 'Änderungen speichern', 'buddydrive' ); ?>" />
			</p>
		</form>
	</div>
...
function buddydrive_handle_settings_in_multisite() {
	...
	?>
	<div id="message" class="updated"><p><?php _e( 'Einstellungen gespeichert', 'buddydrive' );?></p></div>
	<?php
