...
function buddydrive_files_admin_load() {
	...
	if ( 'do_delete' == $doaction && ! empty( $_GET['bid'] ) ) {
		...
		get_current_screen()->add_help_tab( array(
			...
			'title'   => __( 'Überblick', 'buddydrive' ),
			'content' =>
				'<p>' . __( 'Diese Seite ist eine bequeme Möglichkeit, die mit einer Deiner Dateien oder Ordnern verknüpften Details zu bearbeiten.', 'buddydrive' ) . '</p>' .
				'<p>' . __( 'Das Feld Name und Beschreibung ist fixiert, aber Du kannst alle anderen Felder per Drag & Drop neu positionieren und durch Klicken auf die Titelleiste jedes Felds minimieren oder erweitern. Verwende die Registerkarte Bildschirmoptionen, um ein- oder auszublenden oder ein 1- oder 2-spaltiges Layout für diesen Bildschirm auszuwählen.', 'buddydrive' ) . '</p>'
		) );

		// Register metaboxes for the edit screen.
		add_meta_box( 'submitdiv', _x( 'Speichern', 'buddydrive-item admin edit screen', 'buddydrive' ), 'buddydrive_admin_edit_metabox_status', get_current_screen()->id, 'side', 'high' );
		add_meta_box( 'buddydrive_item_privacy', _x( 'Privatsphäre', 'buddydrive-item admin edit screen', 'buddydrive' ), 'buddydrive_admin_edit_metabox_privacy', get_current_screen()->id, 'side', 'core' );
		add_meta_box( 'buddydrive_item_children', _x( 'Dateien', 'buddydrive-item admin edit screen', 'buddydrive' ), 'buddydrive_admin_edit_metabox_list_files', get_current_screen()->id, 'normal', 'core' );
...
function buddydrive_files_admin_index() {
	...
	if ( ! empty( $_REQUEST['deleted'] ) ) {
		...
		if ( $deleted > 0 ) {
			$messages[] = sprintf( _n( '%s Datei wurde endgültig gelöscht.', '%s Dateien wurden endgültig gelöscht.', $deleted, 'buddydrive' ), number_format_i18n( $deleted ) );
		}
	}
	...
	<div class="wrap">
		...
		<h2>
			<?php _e( 'PS Buddy Drive Dateien', 'buddydrive' ); ?>

			<?php if ( !empty( $_REQUEST['s'] ) ) : ?>
				<span class="subtitle"><?php printf( __( 'Suchergebnisse für &#8220;%s&#8221;', 'buddydrive' ), wp_html_excerpt( esc_html( stripslashes( $_REQUEST['s'] ) ), 50 ) ); ?></span>
			<?php endif; ?>
		</h2>
		...
		<form id="buddydrive-form" action="" method="get">
			<?php $buddydrive_list_table->search_box( __( 'Alle Dateien durchsuchen', 'buddydrive' ), 'buddydrive-files' ); ?>
...
function buddydrive_files_admin_edit() {
	...
	if ( $is_error ) {
		$messages[] = __( 'Beim Aktualisieren Deiner Dateidetails ist ein Fehler aufgetreten.', 'buddydrive' );
	} else if ( ! empty( $updated ) ) {
		$messages[] = __( 'Die Datei wurde erfolgreich aktualisiert.', 'buddydrive' );
	}
	...
	<div class="wrap" id="buddydrive-admin-item">
		<?php screen_icon( 'buddydrive' ); ?>
		<h2><?php _e( 'Buddy Drive-Datei bearbeiten', 'buddydrive' ); ?></h2>
						...
						<div id="post-body-content">
							<div id="postdiv" class="postarea">
								<div id="buddydrive_item_name" class="postbox">
									<h3><?php _e( 'Name und Beschreibung', 'buddydrive' ); ?></h3>
									<div class="inside">
										...
										<?php if( $item->post_type == buddydrive_get_file_post_type() ) :?>
											<label for="buddydrive-item-content"><?php _e( 'Beschreibung', 'buddydrive' );?></label>
											<textarea name="buddydrive-edit[item-content]" id="buddydrive-item-content" placeholder="<?php _e('140 Zeichen verfügbar', 'buddydrive')?>" maxlength="140"><?php echo wp_kses( stripslashes( $item->content ), array() );?></textarea>
		...
		<?php else : ?>
			<p><?php printf( __( 'Keine Datei mit dieser ID gefunden. <a href="%s">Geh zurück und versuche es noch einmal</a>.', 'buddydrive' ), esc_url( bp_get_admin_url( 'admin.php?page=buddydrive-files' ) ) ); ?></p>
		<?php endif; ?>
	...
	<div id="submitcomment" class="submitbox">
		<div id="major-publishing-actions">
			<div id="delete-action">
				<a class="submitdelete deletion" href="<?php echo esc_url( wp_nonce_url( add_query_arg( 'action', 'delete', $base_url ), 'buddydrive-item-delete' ) ); ?>"><?php esc_html_e( 'Datei löschen', 'buddydrive' ) ?></a>
			</div>

			<div id="publishing-action">
				<?php submit_button( __( 'Änderungen speichern', 'buddydrive' ), 'primary', 'save', false, array( 'tabindex' => '4' ) ); ?>
			</div>
			<div class="clear"></div>
...
function buddydrive_admin_edit_metabox_privacy( $item ) {
	...
	<div class="buddydrive-owner-section" id="buddydrive-owner-section-info">
		<div>
			<label><?php _e( 'Besitzer', 'buddydrive' );?></label>
			<table>
				<tr>
					<td><?php echo $avatar;?></td>
					<td><?php _e('Buddy Drive-Nutzung', 'buddydrive');?> : <?php buddydrive_user_used_quota( false, $owner );?> %</td>
				</tr>
			</table>
		</div>
	...
	<?php if( empty( $item->post_parent) ) :?>
		...
		<div class="buddydrive-privacy-section" id="buddydrive-privacy-section-options">
			<label for="buddydrive-sharing-option"><?php _e( 'Optionen zum Teilen von Dateien', 'buddydrive' );?></label>
			...
			<div id="buddydrive-admin-privacy-detail">
				<?php if( $privacy_status == 'password' ):?>
					<label for="buddydrive-password"><?php _e( 'Passwort', 'buddydrive' );?></label>
				...
				<?php elseif( $privacy_status == 'groups') :?>
					<label for="buddygroup"><?php _e( 'Wähle die Gruppe', 'buddydrive' );?></label>
		...
		<?php if ( empty( $privacy_status ) ): ?>
			<p><strong><?php _e( 'Der Datenschutz dieser Datei ist nicht definiert, bitte korrigiere dies!', 'buddydrive' );?></strong></p>
		<?php endif; ?>
	...
	<?php else:?>
		...
		<div class="buddydrive-privacy-section" id="buddydrive-privacy-section-options">
			<label for="buddydrive-sharing-option"><?php _e('Optionen zum Teilen von Dateien', 'buddydrive');?></label>
			<p><?php printf( __( "Die Privatsphäre dieser Datei hängt von seinem übergeordneten <a href=\"%s\">Ordner</a> ab", "buddydrive"), esc_url( add_query_arg( array( 'page' => 'buddydrive-files', 'bid' => $item->post_parent, 'action' => 'edit'), bp_get_admin_url( 'admin.php' ) ) ) );?></p>
		</div>
	...
	<?php if( $item->post_type == buddydrive_get_file_post_type() ) :?>
		...
		<div class="buddydrive-folder-section" id="buddydrive-folder-section-options">
			<label for="buddydrive-folder-option"><?php _e( 'Ordner', 'buddydrive' );?></label>
			...
			<tr id="item-<?php buddydrive_item_id();?>">
				...
				<td>
					...
					<div class="row-actions">
						...
						?>
						<span class="edit">
							<a href="<?php echo esc_url( $edit_url );?>"><?php esc_html_e( 'Bearbeiten', 'buddydrive' );?></a> |
						</span>
						<span class="delete">
							<a href="<?php echo esc_url( $delete_url );?>"><?php esc_html_e( 'Löschen', 'buddydrive' );?></a>
						</span>
					</div>
				</td>
				<td>
		...
		<?php if ( buddydrive_has_more_items() ):?>
			<tr>
				<td class="buddydrive-load-more" colspan="4">
					<a href="#more-buddydrive"><?php _e( 'Mehr laden', 'buddydrive' ); ?></a>
				</td>
			</tr>
		<?php endif;?>

	<?php else:?>
		<tr><td colspan="4"><?php _e( 'Keine Dateien an diesen Ordner angehängt', 'buddydrive' );?></td></tr>
	<?php endif;
}
...
function buddydrive_admin_edit_metabox_list_files( $item ) {
	...
	<div class="buddydrive-children-section" id="buddydrive-children-section-files">
			...
			<div class="alignleft actions">
				<select name="action2">
					<option value="-1" selected="selected"><?php _e( 'Massenaktionen', 'buddydrive' );?></option>
					<option value="delete"><?php _e( 'Löschen', 'buddydrive');?></option>
				</select>
				<input type="submit" name="" id="doaction" class="button action" value="Apply">
			</div>
			<table class="widefat" id="buddydrive-admin-files" data-folder="<?php echo $item->ID;?>">
				<thead>
					<tr><th colspan="2"><?php _e( 'Name', 'buddydrive');?></th><th><?php _e( 'Mime Typ', 'buddydrive' );?></th><th><?php _e( 'Letzte Änderung', 'buddydrive' );?></th></tr>
				</thead>
				<tbody>
	...
	<?php else :?>
		<div class="buddydrive-children-section" id="buddydrive-children-section-files">
			<?php _e( 'Das angezeigte Element ist eine Datei, die Dateiliste ist nur für einen Ordner verfügbar', 'buddydrive' );?>
		</div>
	<?php endif; ?>
...
function buddydrive_files_admin_delete(){
	...
	<div class="wrap">
		<?php screen_icon( 'buddydrive' ); ?>
		<h2><?php _e( 'Dateien löschen', 'buddydrive' ); ?></h2>
		<p><?php _e( 'Du bist im Begriff, die folgenden Dateien zu löschen:', 'buddydrive' ); ?></p>
		...
		<?php foreach ( $items as $item ) : ?>
			<li>
				...
				<?php if( $item->post_type == buddydrive_get_folder_post_type() ):?>
					<?php _e('(und alle Dateien dieses Ordners)', 'buddydrive');?>
				<?php endif;?>
			</li>
		<?php endforeach; ?>
		</ul>

		<p><strong><?php _e( 'Diese Aktion kann nicht rückgängig gemacht werden.', 'buddydrive' ); ?></strong></p>

		<a class="button-primary" href="<?php echo esc_url( wp_nonce_url( add_query_arg( array( 'action' => 'do_delete', 'bid' => implode( ',', $bids ) ), $base_url ), 'buddydrive-delete' ) ); ?>"><?php esc_html_e( 'Dauerhaft löschen', 'buddydrive' ); ?></a>
		<a class="button" href="<?php echo esc_url( $base_url ); ?>"><?php esc_html_e( 'Abbrechen', 'buddydrive' ); ?></a>
	</div>
...
class BuddyDrive_List_Table extends WP_List_Table {
	 ...
	 */
	function no_items() {
		_e( 'Keine Elemente gefunden.', 'buddydrive' );
	}
	...
	*/
	...
	function get_views() {
		...
		<ul class="subsubsub">
			<li class="all"><a href="<?php echo esc_url( $url_base ); ?>" class="<?php if ( 'all' == $this->view ) echo 'current'; ?>"><?php _e( 'Alle', 'buddydrive' ); ?></a> |</li>
			<li class="public"><a href="<?php echo esc_url( add_query_arg( 'buddydrive_type', buddydrive_get_file_post_type(), $url_base ) ); ?>" class="<?php if ( buddydrive_get_file_post_type() == $this->view ) echo 'current'; ?>"><?php _e( 'Dateien', 'buddydrive' ) ; ?></a> |</li>
			<li class="private"><a href="<?php echo esc_url( add_query_arg( 'buddydrive_type', buddydrive_get_folder_post_type(), $url_base ) ); ?>" class="<?php if ( buddydrive_get_folder_post_type() == $this->view ) echo 'current'; ?>"><?php  _e( 'Ordner', 'buddydrive' ); ?></a></li>
	...
	function get_columns() {
		return apply_filters( 'buddydrive_list_table_columns', array(
			...
			'description' => _x( 'Beschreibung', 'BuddyDrive admin Item Description column header', 'buddydrive' ),
			'status'      => _x( 'Privatsphäre', 'BuddyDrive admin Privacy Status column header',       'buddydrive' ),
			'owner'       => _x( 'Besitzer', 'BuddyDrive admin Owner column header',                  'buddydrive' ),
			'mime_type'   => _x( 'Mime Typ', 'BuddyDrive admin Mime type column header',          'buddydrive' ),
			'last_edit'   => _x( 'Letzte Änderung', 'BuddyDrive admin Last Edit column header',          'buddydrive' )
		));
	}
	...
	function column_comment( $item = array() ) {
		...
		// Edit
		$actions['edit'] = sprintf( '<a href="%s">%s</a>', esc_url( $edit_url ), __( 'Bearbeiten', 'buddydrive' ) );

		// Delete
		$actions['delete'] = sprintf( '<a href="%s">%s</a>', esc_url( $delete_url ), __( 'Löschen', 'buddydrive' ) );
	...
	function column_status( $item = array() ) {
		...
		if( !empty( $privacy ) ) {
			switch ( $privacy ) {
				case 'private' :
					$status_desc = '<i class="icon bd-icon-lock"></i> ' . __( 'Privat', 'buddydrive' );
					break;

				case 'password' :
					$status_desc = '<i class="icon bd-icon-key"></i> ' . __( 'Passwortgeschützt', 'buddydrive' );
					break;

				case 'public'  :
					$status_desc = '<i class="icon bd-icon-unlocked"></i> ' . __( 'Öffentlich', 'buddydrive' );
					break;

				case 'friends'  :
					$status_desc = '<i class="icon bd-icon-users"></i> ' . __( 'Nur Freunde', 'buddydrive' );
					break;
