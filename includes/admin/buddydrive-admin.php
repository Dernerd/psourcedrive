...
class BuddyDrive_Admin {
	...
	public function admin_menus() {
		...
		$this->hook_suffixes[] = add_submenu_page(
			$this->settings_page,
			_x( 'PS Buddy Drive', 'BuddyDrive Settings page title', 'buddydrive' ),
			_x( 'PS Buddy Drive', 'BuddyDrive Settings menu title', 'buddydrive' ),
			'manage_options',
			'buddydrive',
		...
		$hook = add_menu_page(
			_x( 'PS Buddy Drive', 'BuddyDrive User Files Admin page title', 'buddydrive' ),
			_x( 'PS Buddy Drive', 'BuddyDrive User Files Admin menu title',  'buddydrive' ),
			'manage_options',
			'buddydrive-files',
		...
		$this->hook_suffixes[] = add_dashboard_page(
			__( 'Willkommen bei PS Buddy Drive',  'buddydrive' ),
			__( 'Willkommen bei PS Buddy Drive',  'buddydrive' ),
			'manage_options',
			'buddydrive-about',
		...
		if ( $this->requires_db_upgrade ) {
			$this->hook_suffixes['upgrade'] = add_dashboard_page(
				__( 'PS Buddy Drive Upgrades',  'buddydrive' ),
				__( 'PS Buddy Drive Upgrades',  'buddydrive' ),
				'manage_options',
				'buddydrive-upgrade',
	...
	public function activation_notice() {
		...
		if ( empty( $buddydrive_upload['dir'] ) || ! file_exists( $buddydrive_upload['dir'] ) ){
			bp_core_add_admin_notice( __( 'Das Hauptverzeichnis von PS Buddy Drive fehlt', 'buddydrive' ) );
		}
		...
		if ( $this->requires_db_upgrade && $display_upgrade_notice ) {
			bp_core_add_admin_notice( sprintf(
				__( 'PS Buddy Drive ist fast fertig. Es muss einige der verwendeten Daten aktualisieren. Wenn Du noch keine Datenbanksicherung durchgeführt hast, führe dies bitte <strong>vorher</strong> durch, indem Du auf <a href="%s">diesen Link</a> klickst.', 'buddydrive' ),
	...
	public function modify_plugin_action_links( $links, $file ) {
		...
		return array_merge( $links, array(
			'settings' => '<a href="' . esc_url( add_query_arg( array( 'page' => 'buddydrive'       ), bp_get_admin_url( $this->settings_page ) ) ) . '">' . esc_html__( 'Einstellungen', 'buddydrive' ) . '</a>',
			'about'    => '<a href="' . esc_url( add_query_arg( array( 'page' => 'buddydrive-about' ), bp_get_admin_url( 'index.php'          ) ) ) . '">' . esc_html__( 'Über',    'buddydrive' ) . '</a>'
		) );
	}
	...
	public function about_screen() {
		...
		<div class="wrap about-wrap">
			<h1><?php printf( __( 'PS Buddy Drive %s', 'buddydrive' ), $display_version ); ?></h1>
			<div class="about-text"><?php printf( __( 'Vielen Dank für das Upgrade auf die neueste Version von PS Buddy Drive! PS Buddy Drive %s ist bereit, die Dateien und Ordner Deiner Freunde zu verwalten!', 'buddydrive' ), $display_version ); ?></div>
			...
			<h2 class="nav-tab-wrapper">
				<a class="nav-tab nav-tab-active" href="<?php echo esc_url(  bp_get_admin_url( add_query_arg( array( 'page' => 'buddydrive-about' ), 'index.php' ) ) ); ?>">
					<?php _e( 'Über', 'buddydrive' ); ?>
				</a>
			</h2>

			<div class="headline-feature feature-section one-col">
				<h2><?php _e( 'Dies ist die neue PS Buddy Drive-Benutzeroberfläche', 'buddydrive' ); ?></h2>

				<div class="media-container" style="text-align:center">
					<img src="<?php echo esc_url( buddydrive_get_images_url() . '/buddydrive-ui.gif' );?>" alt="<?php esc_attr_e( 'Der PS Buddy Drive-Editor', 'buddydrive' ); ?>">
				</div>

				<div class="col" style="margin-right: auto;margin-left: auto; float: none">
					<h4><?php _e( 'Die Benutzeroberfläche wurde komplett überarbeitet und bringt mehrere Datei-Uploads!', 'buddydrive' ); ?></h4>
					<p><?php _e( 'Das Hochladen von Dateien war noch nie so einfach! Ziehe die hochgeladenen Dateien per Drag&Drop. Der Standard-Datenschutz entspricht nicht Deinen Anforderungen? Keine Sorge, Du kannst es jederzeit bearbeiten!', 'buddydrive' ); ?></p>
				</div>
			...
			<div class="feature-section two-col">
				<h2><?php _e( 'Verbesserungen bei der BuddyPress Gruppen-Integration', 'buddydrive' ); ?></h2>
				<div class="col">
					<img src="<?php echo buddydrive_get_plugin_url();?>/screenshot-1.png">
					<h3><?php _e( 'Mit mehreren Gruppen teilen', 'buddydrive' ); ?></h3>
					<p><?php _e( 'Hast Du dieselbe Datei schon mehrmals hochgeladen, um sie mit verschiedenen Gruppen zu teilen? Das war vorher! Jetzt kannst Du eine Datei an beliebig viele Gruppen anhängen.', 'buddydrive' ); ?></p>
				</div>
				<div class="col">
					<img src="<?php echo buddydrive_get_plugin_url();?>/screenshot-2.png">
					<h3><?php _e( 'Erstelle neue Dateien direkt aus der Gruppe.', 'buddydrive' ); ?></h3>
					<p><?php _e( 'Bist Du es leid, zu Deinem Profil zurückzukehren, um Elemente innerhalb einer Gruppe zu teilen? Das ist Geschichte! Jetzt kannst Du Ordner erstellen und neue Dateien direkt aus der Gruppe hochladen.', 'buddydrive' ); ?></p>
				</div>
			</div>
			...
			<div class="feature-section two-col">
				<h2><?php _e( 'Verbesserungen beim Teilen', 'buddydrive' ); ?></h2>
				<div class="col">
					<img src="<?php echo buddydrive_get_plugin_url();?>/screenshot-3.png">
					<h3><?php _e( 'Die Mitglieder Deiner Wahl!', 'buddydrive' ); ?></h3>
					<p><?php _e( 'Jetzt kannst Du den Zugriff auf Deine Ordner und Dateien auf die von Dir gewählte Auswahl beschränken! Finde alle Dateien und Ordner, die der andere mit Dir geteilt hat, in der neuen &quot;Zwischen Mitgliedern&quot; Registerkarte Deines PS Buddy Drive.', 'buddydrive' ); ?></p>
				</div>
				<div class="col">
					<img src="<?php echo buddydrive_get_plugin_url();?>/screenshot-4.png">
					<h3><?php _e( 'Echte freigegebene Ordner', 'buddydrive' ); ?></h3>
					<p><?php _e( 'Der Benutzer kann auf Deinen Ordner zugreifen? Jetzt kann er auch neue Dateien darin hinzufügen. Der Ordnerbesitzer hat immer noch das letzte Wort und kann alle hinzugefügten Dateien entfernen.', 'buddydrive' ); ?></p>
				</div>
			</div>
			...
			<div class="changelog">
				<h2><?php printf( __( 'Die anderen Verbesserungen in %s', 'buddydrive' ), $display_version ); ?></h2>

				<div class="under-the-hood three-col">
					<div class="col">
						<h3><?php _e( 'Administratorrechte', 'buddydrive' ); ?></h3>
						<p><?php _e( 'Administratoren können jetzt die Dateien und Ordner eines bestimmten Benutzers über den PS Buddy Drive-Verwaltungsbildschirm durchsuchen und beliebige Dateien oder Ordner hinzufügen/bearbeiten oder entfernen.', 'buddydrive' ); ?></p>
					</div>
					<div class="col">
						<h3><?php _e( 'Suche', 'buddydrive' ); ?></h3>
						<p><?php _e( 'Der Benutzeroberfläche wurde ein neues Suchfeld hinzugefügt, damit Du Deine alten Dateien leicht finden kannst!', 'buddydrive' ); ?></p>
					</div>
					<div class="col">
						<h3><?php _e( 'Detaillierte Benutzerstatistiken', 'buddydrive' ); ?></h3>
						<p><?php _e( 'Zusätzlich zur Festplattennutzung werden detaillierte Statistiken angezeigt, um die Verteilung der Anzahl der Dateien nach Sichtbarkeit anzugeben.', 'buddydrive' ); ?></p>
					</div>
				</div>
			</div>

			<div class="return-to-dashboard">
				<a href="<?php echo esc_url( $settings_url );?>" title="<?php esc_attr_e( 'PS Buddy Drive konfigurieren', 'buddydrive' ); ?>"><?php esc_html_e( 'Gehe zur Seite PS Buddy Drive-Einstellungen', 'buddydrive' );?></a>
			</div>
	...
	public static function edit_user_quota( $profileuser ) {
		...
		<h3><?php esc_html_e( 'PS Buddy Drive-Kontingent des Nutzers', 'buddydrive' ); ?></h3>

		<table class="form-table">
			<tbody>
				<tr>
					<th><label for="_buddydrive_user_quota"><?php esc_html_e( 'Speicherplatz verfügbar', 'buddydrive' ); ?></label></th>
					<td>
						...
						<label for="_buddydrive_user_quota"><?php _e( 'MB', 'buddydrive' ); ?></label>
					</td>
				</tr>
	...
	public static function user_quota_column( $columns = array() ) {
		$columns['user_quota'] = __( 'PS Buddy Drive-Nutzung',  'buddydrive' );
		...
		if ( 'user_quota' === $column_name && ! empty( $user_id ) ) {
			...
			if ( ! empty( $quota['percent'] ) && 0 < (float) $quota['percent'] ) {
				$retval = sprintf(
					...
					esc_attr__( 'Alle Dateien für diesen Benutzer anzeigen', 'buddydrive' ),
					$quota['percent'] . '%'
				);
			} else {
	...
	public function upgrade_screen() {
		...
		?>
		<div class="wrap">
			<h1><?php esc_html_e( 'PS Buddy Drive Upgrade', 'buddydrive' ); ?></h1>
			<div id="message" class="fade updated buddydrive-hide">
				<p><?php esc_html_e( 'Vielen Dank für Deine Geduld, Du kannst PS Buddy Drive jetzt in vollem Umfang genießen!', 'buddydrive' ); ?></p>
			</div>
			<p>
		<?php
		...
		if ( ! isset( $tasks ) || empty( $tasks ) ) {
			esc_html_e( 'Keine auszuführenden Aufgaben. PS Buddy Drive ist bereit.', 'buddydrive' );
		} else {
			...
			printf( _n( 'PS Buddy Drive ist fast fertig, bitte warte, bis die folgende Aufgabe von %s fortgeführt wird.', 'PS Buddy Drive ist fast fertig, bitte warte, bis die folgenden %s Aufgaben fortgeführt werden.', count( $tasks ), 'buddydrive' ), number_format_i18n( count( $tasks ) ) );
		}
		?>
			</p>
	...
	public function do_upgrade() {
		$error = array(
			'message'   => __( 'Die Aufgabe konnte aufgrund eines Fehlers nicht verarbeitet werden', 'buddydrive' ),
			'type'      => 'error'
		);
		...
		// Processing any other tasks
			...
			if ( 0 === $did && ( (int) $_POST['count'] > ( (int) $_POST['done'] + (int) $did ) ) ) {
				wp_send_json_error( array( 'message' => __( '%d Dateien konnten nicht aktualisiert werden', 'buddydrive' ), 'type' => 'warning', 'action_id' => $_POST['id'] ) );
			}
		} else {
			wp_send_json_error( $error );
		}
	...
	public function items_admin_screen() {
		?>
		<div class="wrap">
			<h1><?php esc_html_e( 'PS Buddy Drive Dateien', 'buddydrive' ); ?></h1>
