...
function buddydrive_get_js_l10n() {
	$buddydrivel10n = array(
		'one_at_a_time'        => __( 'Bitte füge jeweils nur eine Datei hinzu', 'buddydrive' ),
		'loading'              => __( 'laden..', 'buddydrive' ),
		'shared'               => __( 'Geteilt', 'buddydrive' ),
		'group_remove_error'   => __( 'Fehler: Artikel konnte nicht aus der aktuellen Gruppe entfernt werden', 'buddydrive' ),
		'cbs_message'          => __( 'Bitte verwende die Kontrollkästchen, um einen oder mehrere Artikel auszuwählen', 'buddydrive' ),
		'cb_message'           => __( 'Bitte verwende das Kontrollkästchen, um ein Element zum Bearbeiten auszuwählen', 'buddydrive' ),
		'confirm_delete'       => __( 'Möchtest Du %d Artikel wirklich löschen?', 'buddydrive' ),
		'delete_error_message' => __( 'Fehler: Artikel konnten nicht gelöscht werden', 'buddydrive' ),
		'title_needed'         => __( 'Der Titel ist erforderlich', 'buddydrive' ),
		'group_needed'         => __( 'Bitte wähle eine Gruppe aus der Liste', 'buddydrive' ),
		'pwd_needed'           => __( 'Bitte wähle ein Passwort', 'buddydrive' ),
		'define_pwd'           => __( 'Definiere dein Passwort', 'buddydrive' ),
		'label_pwd'            => __( 'Passwort', 'buddydrive' ),
		'label_group'          => __( 'Wähle die Gruppe', 'buddydrive' ),
	);
...
class BuddyDrive_Uploader {
	...
	public function enqueue_scripts() {
		...
		$pluploadmessages = array(
				'queue_limit_exceeded'      => __( 'Du hast versucht, zu viele Dateien in die Warteschlange zu stellen.', 'buddydrive' ),
				'file_exceeds_size_limit'   => __( '%s überschreitet die maximale Upload-Größe für diese Webseite.', 'buddydrive' ),
				'zero_byte_file'            => __( 'Diese Datei ist leer. Bitte versuche es mit einer anderen.', 'buddydrive' ),
				'invalid_filetype'          => __( 'Dieser Dateityp ist nicht zulässig. Bitte versuche es mit einem anderen.', 'buddydrive' ),
				'not_an_image'              => __( 'Diese Datei ist kein Bild. Bitte versuche es mit einem anderen.', 'buddydrive' ),
				'image_memory_exceeded'     => __( 'Speicher überschritten. Bitte versuche es mit einer anderen kleineren Datei.', 'buddydrive' ),
				'image_dimensions_exceeded' => __( 'Dies ist größer als die maximale Größe. Bitte versuche es mit einem anderen.', 'buddydrive' ),
				'default_error'             => __( 'Beim Hochladen ist ein Fehler aufgetreten. Bitte versuche es später noch einmal.', 'buddydrive' ),
				'missing_upload_url'        => __( 'Es ist ein Konfigurationsfehler aufgetreten. Bitte wende Dich an den Serveradministrator.', 'buddydrive' ),
				'upload_limit_exceeded'     => __( 'Du kannst nur 1 Datei hochladen.', 'buddydrive' ),
				'http_error'                => __( 'HTTP Fehler.', 'buddydrive' ),
				'upload_failed'             => __( 'Upload fehlgeschlagen.', 'buddydrive' ),
				'big_upload_failed'         => __( 'Bitte versuche, diese Datei mit dem %1$sBrowser-Uploader%2$s hochzuladen.', 'buddydrive' ),
				'big_upload_queued'         => __( '%s überschreitet die maximale Upload-Größe für den Multi-File-Uploader, wenn er in Deinem Browser verwendet wird.', 'buddydrive' ),
				'io_error'                  => __( 'IO Fehler.', 'buddydrive' ),
				'security_error'            => __( 'Sicherheitsfehler.', 'buddydrive' ),
				'file_cancelled'            => __( 'Datei abgebrochen.', 'buddydrive' ),
				'upload_stopped'            => __( 'Upload gestoppt.', 'buddydrive' ),
				'dismiss'                   => __( 'Zurückweisen', 'buddydrive' ),
				'crunching'                 => __( 'Knirschen&hellip;', 'buddydrive' ),
				'deleted'                   => __( 'In den Papierkorb verschoben.', 'buddydrive' ),
				'error_uploading'           => __( '&#8220;%s&#8221; konnte nicht hochladen.', 'buddydrive' )
			);
	...
	public function display() {
		...
		<form enctype="multipart/form-data" method="post" action="" class="media-upload-form type-form validate standard-form" id="file-form">
			...
			if ( ! _device_can_upload() ) {
				echo '<p>' . __( 'Der Webbrowser auf Deinem Gerät kann nicht zum Hochladen von Dateien verwendet werden. Möglicherweise kannst Du stattdessen die <a href="http://wordpress.org/extend/mobile/">native App für Dein Gerät</a> verwenden.', 'buddydrive' ) . '</p>';
				return;
			}
			...
			<div id="buddydrive-first-step" class="buddydrive-step">
				<label for="buddyfile-desc"><?php _e( 'Beschreibe Deine Datei', 'buddydrive' );?></label>
				<textarea placeholder="<?php _e( '140 Zeichen verfügbar', 'buddydrive' );?>" maxlength="140" id="buddyfile-desc"></textarea>
				<p class="buddydrive-action"><a href="#" class="next-step button"><?php _e( 'Nächster Schritt', 'buddydrive' );?></a></p>
			</div>
			...
			<div id="buddydrive-second-step" class="buddydrive-step hide">
				<label for="buddydrive-sharing-options"><?php _e( 'Definiere Deine Freigabeoptionen', 'buddydrive' );?></label>
			...
			$plupload_init = array(
				...
				'filters'             => array(
					array(
						'title'         => __( 'Zulässige Dateien', 'buddydrive' ),
						'extensions'    => '*',
			...
			<div id="buddydrive-third-step" class="buddydrive-step hide">
				<label for="plupload-browse-buttons"><?php _e( 'Lade Deine Datei hoch!', 'buddydrive' );?></label>

				<div id="plupload-upload-ui" class="hide-if-no-js">

					<div id="drag-drop-area">
						<div class="drag-drop-inside">
							<p class="drag-drop-info"><?php _e( 'Lege Deine Datei hier ab', 'buddydrive' ); ?></p>
							<p><?php _ex( 'oder', 'Uploader: Lege Deine Datei hier ab - oder - Wähle Deine Datei', 'buddydrive' ); ?></p>
							<p class="drag-drop-buttons"><input id="plupload-browse-button" type="button" value="<?php esc_attr_e( 'Wähle Deine Datei', 'buddydrive' ); ?>" class="button" /></p>
						</div>
					</div>

				</div>

				<p><span class="max-upload-size"><?php printf( __( 'Maximale Upload-Dateigröße: %d%s.', 'buddydrive' ), esc_html( $upload_size_unit ), esc_html( $sizes[$u] ) ); ?></span></p>
				<p class="buddydrive-action"><a href="#" class="cancel-step button"><?php _e( 'Abbrechen', 'buddydrive' );?></a></p>
...
function buddydrive_attached_file_to_message() {
	...
	if ( ! empty( $_REQUEST['buddyitem'] ) ) {
		...
		if ( ! empty( $buddyitem->ID ) ){
			...
			?>
			<p>
				<label for="buddyitem-link"><?php printf( __( '%s angehängt : %s', 'buddydrive' ), esc_html( $buddytype ), '<a href="' . esc_url( $displayed_link ) . '">'. esc_html( $buddyitem->title ). '</a>' );?></label>
				...
				<?php if ( ! empty( $password ) ) :?>
					<input type="checkbox" name="_buddyitem_pass" value="1" checked> <?php _e('Füge das Passwort automatisch in die Nachricht ein', 'buddydrive');?>
				<?php endif;?>
			</p>
			<?php
		}
...
function buddydrive_update_message_content( $message ) {
	...
	if ( ! empty( $_POST['_buddyitem_link'] ) ){
		...
		if ( ! empty( $_POST['_buddyitem_pass'] ) ) {
			...
			$password = ! empty( $password_check ) ? '<p>'.sprintf( __('Passwort : %s', 'buddydrive'), $password_check ) .'</p>' : false;
...
function buddydrive_save_new_buddyfile() {
	...
	if ( ! empty( $buddydrive_file ) && is_array( $buddydrive_file ) && empty( $buddydrive_file['error'] ) ) {
		...
		echo '<div class="error-div"><a class="dismiss" href="#">' . __( 'Verwerfen', 'buddydrive' ) . '</a><strong>' . sprintf( __( '&#8220;%s&#8221; konnte aufgrund eines Fehlers nicht hochgeladen werden: %s', 'buddydrive' ), esc_html( $_FILES['buddyfile-upload']['name'] ), $buddydrive_file['error'] ) . '</strong><br /></div>';
	}
...
function buddydrive_open_buddyfolder() {
	...
	if ( empty( $buddyfolder->ID ) ) {
		...
		$result[] = '<tr id="no-buddyitems"><td colspan="5"><div id="message" class="info"><p>'. __( 'Dieser Ordner existiert leider nicht mehr.', 'buddydrive' ).'</p></div></td></tr>';
		...
		if ( ! empty( $access ) || bp_current_user_can( 'bp_moderate' ) ) {
			...
			$result[] = '<tr id="no-access"><td colspan="5"><div id="message" class="info"><p>'. __( 'Entschuldigung, dieser Ordner ist privat', 'buddydrive' ).'</p></div></td></tr>';
		...
		<form class="standard-form" id="buddydrive-item-edit-form">
			...
			<div id="buddyitem-description">
				...
				<?php if ( $item->post_type == buddydrive_get_file_post_type() ) :?>
					<label for="buddydrive-item-content"><?php esc_html_e( 'Beschreibung', 'buddydrive' );?></label>
			...
			<?php if ( empty( $item->post_parent ) ) :?>
				...
				<div id="buddydrive-privacy-section-options">
					<label for="buddydrive-sharing-option"><?php esc_html_e( 'Optionen zum Teilen von Dateien', 'buddydrive' );?></label>
					...
					<div id="buddydrive-admin-privacy-detail">
						<?php if( $item->check_for == 'password' ):?>
							<label for="buddydrive-password"><?php esc_html_e( 'Passwort', 'buddydrive' );?></label>
						...
						<?php elseif( $item->check_for == 'groups' ) :?>
							<label for="buddygroup"><?php esc_html_e( 'Wähle die Gruppe', 'buddydrive' );?></label>
			...
			<?php else :?>
				...
				<div id="buddyitem-sharing-option">
					<label for="buddydrive-sharing-option"><?php esc_html_e( 'Bearbeite Deine Freigabeoptionen', 'buddydrive' );?></label>
					<p><?php esc_html_e( 'Die Privatsphäre dieses Elements hängt von seinem übergeordneten Ordner ab', 'buddydrive' );?></p>
				</div>
			...
			<?php if ( $item->post_type == buddydrive_get_file_post_type() ) :?>
				...
				<div class="buddyitem-folder-section" id="buddyitem-folder-section-options">
					<label for="buddyitem-folder-option"><?php esc_html_e( 'Ordner', 'buddydrive' );?></label>
					<?php buddydrive_select_folder_options( $item->user_id, $item->post_parent, 'buddydrive-edit[folder]' );?>
				</div>

			<?php endif;?>

			<p class="buddydrive-action folder"><input type="submit" value="<?php esc_html_e('Objekt bearbeiten', 'buddydrive');?>" name="buddydrive_edit[submit]">&nbsp;<a href="#" class="cancel-item button"><?php esc_html_e( 'Abbrechen', 'buddydrive' );?></a></p>
...
function buddydrive_share_in_group() {
	...
	if ( empty( $buddyitem ) ) {
		_e( 'das ist peinlich, hat nicht geklappt:(', 'buddydrive' );
		die();
	}

	if ( ! bp_is_active( 'groups' ) ) {
		_e( 'Gruppenkomponente ist deaktiviert, bitte kontaktiere den Administrator.', 'buddydrive' );
		die();
	}
	...
	if ( ! empty( $buddyitem ) ) {
		...
		if ( ! empty( $group_id ) ) {
			...
			$action  = $activity_action  = sprintf( __( '%1$s hat ein %2$s Element in der Gruppe %3$s geteilt', 'buddydrive'), bp_core_get_userlink( $user_id ), esc_html( buddydrive_get_name() ), '<a href="' . esc_url( bp_get_group_permalink( $group ) ) . '">' . esc_html( $group->name ) . '</a>' );
			$content = $link;
			$args = array(
					'user_id'   => $user_id,
					'action'    => $action,
...
function buddydrive_share_in_profile() {
	...
	if ( ! empty( $buddyitem ) ) {
		...
		if ( empty( $buddyfile->ID ) || $buddyfile->check_for != 'public' ) {
			...
			_e( 'Wir konnten Dein PS Buddy Drive-Element nicht finden oder sein Datenschutz ist nicht auf öffentlich eingestellt', 'buddydrive');
			die();
		}

		$action  = sprintf( __( '%1$s hat einen %2$s Artikel geteilt', 'buddydrive'), bp_core_get_userlink( $user_id ), buddydrive_get_name() );
		$content = $link;
		$args = array(
				'user_id'      => $user_id,
				'action'       => $action,
	...
	if ( ! empty( $result ) )
		...
		echo _e( 'das ist peinlich, hat nicht geklappt:(', 'buddydrive' );
	die();
...
function buddydrive_user_files_content() {
	...
	<div id="buddydrive-forms">
		<div class="buddydrive-crumbs"><a href="<?php esc_url( buddydrive_component_home_url() );?>" name="home" id="buddydrive-home"><i class="icon bd-icon-root"></i> <span id="folder-0" class="buddytree current"><?php esc_html_e( 'Root-Verzeichnis', 'buddydrive' );?></span></a></div>
...
function buddydrive_group_deprecated_display( $group_id = 0 ) {
	...
	<div class="buddydrive-crumbs in-group">
		<a href="<?php esc_url( buddydrive_component_home_url() );?>" name="home" id="buddydrive-home" data-group="<?php echo esc_attr( $group_id );?>"><i class="icon bd-icon-root"></i> <span id="folder-0" class="buddytree current"><?php _e( 'Root-Verzeichnis', 'buddydrive');?></span></a>
...
function buddydrive_select_folder_options( $user_id = false, $selected = false, $name = false ) {
	...
	function buddydrive_get_select_folder_options( $user_id = false, $selected = false, $name = false ) {
		...
		$output = __( 'Kein Ordner verfügbar', 'buddydrive' );
		...
		if ( buddydrive_has_items( $buddydrive_args ) ) {
			...
			$output .= '<option value="0" '.selected( $selected, 0, false ).'>'. __( 'Root-Verzeichnis', 'buddydrive' ).'</option>';
...
function buddydrive_select_user_group( $user_id = false, $selected = false, $name = false ) {
	...
	function buddydrive_get_select_user_group( $user_id = false, $selected = false, $name = false ) {
		...
		$output = __( 'Keine Gruppe für BuddyDrive verfügbar', 'buddydrive' );
...
function buddydrive_folder_form() {
	...
	<form class="standard-form" action="" method="post" id="buddydrive-folder-editor-form">
		...
		<div id="buddyfolder-first-step">
			<label for="buddyfolder-sharing-options"><?php _e( 'Definiere Deine Freigabeoptionen', 'buddydrive' );?></label>
		...
		<div id="buddyfolder-second-step" class="hide">
			<label for="buddydrive-folder-title"><?php _e( 'Erstelle Deinen Ordner', 'buddydrive' );?></label>
			<input type="text" placeholder="<?php _e( 'Name Deines Ordners', 'buddydrive' );?>" id="buddydrive-folder-title" name="buddydrive_folder[title]">
			<p class="buddydrive-action folder"><input type="submit" value="<?php _e( 'Ordner hinzufügen', 'buddydrive' );?>" name="buddydrive_folder[submit]">&nbsp;<a href="#" class="cancel-folder button"><?php _e( 'Abbrechen', 'buddydrive' );?></a></p>
		</div>
	</form>
	<?php
}
...
function buddydrive_owner_avatar() {
	...
	function buddydrive_get_show_owner_avatar( $user_id = false, $width = '32', $height = '32' ) {
		...
		$avatar  = bp_core_fetch_avatar( array(
			...
			'type'       => 'thumb',
			'avatar_dir' => 'avatars',
			'alt'        => sprintf( __( 'Benutzer-Avatar von %s', 'buddydrive' ), $username ),
			'width'      => $width,
			'height'     => $height,
			'title'      => $username
		) );
...
function buddydrive_group_avatar() {
	...
	function buddydrive_get_group_avatar( $item_id = false, $nolink = false, $width ='32', $height = '32' ) {
		...
		$group_avatar  = bp_core_fetch_avatar( array(
			...
			'alt'        => sprintf( __( 'Gruppenlogo von %d', 'buddydrive' ), esc_attr( $group_name ) ),
			'width'      => $width,
			'height'     => $height,
...
function buddydrive_owner_or_cb() {
	...
	function buddydrive_get_owner_or_cb() {
		...
		if ( bp_is_my_profile() && bp_current_action() == 'files' ) {
			...
			$output = '<a href="' . esc_url( buddydrive_get_owner_link() ) . '" title="'. esc_attr__( 'Besitzer', 'buddydrive' ).'">'. buddydrive_get_show_owner_avatar() . '</a>';
		}
...
function buddydrive_item_privacy() {
	...
	switch ( $status['privacy'] ) {
		case 'private' :
			echo '<a title="'.__( 'Privat', 'buddydrive' ).'"><i class="icon bd-icon-lock"></i></a>';
			break;
		case 'public' :
			echo '<a title="'.__( 'Öffentlich', 'buddydrive' ).'"><i class="icon bd-icon-unlocked"></i></a>';
			break;
		case 'friends' :
			echo '<a title="'.__( 'Freunde', 'buddydrive' ).'"><i class="icon bd-icon-users"></i></a>';
			break;
		case 'password' :
			echo '<a title="'.__( 'Passwortgeschützt', 'buddydrive' ).'"><i class="icon bd-icon-key"></i></a>';
			break;
		case 'groups' :
			if( !empty( $status['group'] ) )
				echo buddydrive_get_group_avatar( buddydrive_get_item_id() );
			else
				_e( 'Gruppe', 'buddydrive' );
			break;
		default:
			/**
...
function buddydrive_item_mime_type() {
	...
	function buddydrive_get_item_mime_type() {
		...
		$mime_type = __( 'Ordner', 'buddydrive' );

		if ( buddydrive_is_buddyfile() ) {
			$doc = $buddydrive_template->query->post->guid;

			$mime_type = __( 'Datei', 'buddydrive' );
	...
	function buddydrive_get_row_actions() {
		...
		if ( buddydrive_is_buddyfile() ) {
			...
			if ( ! empty( $description ) ) {
				$inside_top[]= '<a class="buddydrive-show-desc" href="#">' . __('Beschreibung', 'buddydrive'). '</a>';
		...
		switch ( $privacy['privacy'] ) {
			case 'public':
				...
				if ( buddydrive_current_user_can_share() && bp_is_active( 'activity' ) )
					$inside_top[]= '<a class="buddydrive-profile-activity" href="#">' . __( 'Teilen', 'buddydrive' ). '</a>';
				break;
			case 'password':
				if ( buddydrive_current_user_can_link( $privacy ) ){
					$inside_top[]= '<a class="buddydrive-show-link" href="#">' . __( 'Link', 'buddydrive' ). '</a>';
				...
				if ( buddydrive_current_user_can_share() && bp_is_active( 'messages' ) )
					$inside_top[]= '<a class="buddydrive-private-message" href="'. esc_url( bp_loggedin_user_domain() . bp_get_messages_slug() . '/compose/?buddyitem=' . $buddyfile_id ) . '">' . esc_html__('Teilen', 'buddydrive'). '</a>';
				break;
			case 'friends':
				if ( buddydrive_current_user_can_link( $privacy ) ){
					$inside_top[]= '<a class="buddydrive-show-link" href="#">' . __( 'Link', 'buddydrive' ). '</a>';
				...
				if ( buddydrive_current_user_can_share() && bp_is_active( 'messages' ) )
					$inside_top[]= '<a class="buddydrive-private-message" href="'. esc_url( bp_loggedin_user_domain() . bp_get_messages_slug() . '/compose/?buddyitem='.$buddyfile_id ) . '&friends=1">' . esc_html__( 'Teilen', 'buddydrive' ). '</a>';
				break;
			case 'groups':
				if ( buddydrive_current_user_can_link( $privacy ) ){
					$inside_top[]= '<a class="buddydrive-show-link" href="#">' . __( 'Link', 'buddydrive' ). '</a>';
				...
				if ( buddydrive_current_user_can_share() && bp_is_active( 'activity' ) && bp_is_active( 'groups' ) )
					$inside_top[]= '<a class="buddydrive-group-activity" href="#">' . __( 'Teilen', 'buddydrive' ). '</a>';
				if ( buddydrive_current_user_can_remove( $privacy['group'] ) && bp_is_active( 'groups') )
					$inside_top[]= '<a class="buddydrive-remove-group" href="#" data-group="'. esc_attr( $privacy['group'] ).'">' . esc_html__( 'Entfernen', 'buddydrive' ). '</a>';
				break;
		}
			...
			<ul>
				...
				<?php if ( buddydrive_is_user_buddydrive() ):?>
					...
					<li id="buddydrive-action-new-file">
						<a href="#" id="buddy-new-file" title="<?php _e('Neue Datei', 'buddydrive');?>"><i class="icon bd-icon-newfile"></i></a>
					</li>
					<li id="buddydrive-action-new-folder">
						<a href="#" id="buddy-new-folder" title="<?php _e('Neuer Ordner', 'buddydrive');?>"><i class="icon bd-icon-newfolder"></i></a>
					</li>
					<li id="buddydrive-action-edit-item">
						<a href="#" id="buddy-edit-item" title="<?php _e('Objekt bearbeiten', 'buddydrive');?>"><i class="icon bd-icon-edit"></i></a>
					</li>
					<li id="buddydrive-action-delete-item">
						<a href="#" id="buddy-delete-item" title="<?php _e('Datei löschen', 'buddydrive');?>"><i class="icon bd-icon-delete"></i></a>
					</li>
				...
				<li class="last"><?php esc_html_e( 'Sortieren nach:', 'buddydrive' );?>
					<select name="buddydrive_filter" id="buddydrive-filter">
						<option value="title"><?php esc_html_e( 'Name', 'buddydrive' ) ;?></option>
						<option value="modified"><?php esc_html_e( 'Letzte Änderung', 'buddydrive' ) ;?></option>
					</select>
				</li>
