...
function buddydrive_user_used_quota( $type = false, $user_id = false ) {
	...
	function buddydrive_get_user_space_left( $type = false, $user_id = false ){
		...
		if ( 'diff' === $type ) {
			...
			return apply_filters( 'buddydrive_get_user_space_left', sprintf( __( '<span id="buddy-quota">%s</span>&#37; verwendet', 'buddydrive' ), $space_left['percent'] ), $space_left );
		}
...
function buddydrive_file_password_form() {
	...
	<form action="" method="post" class="standard-form">
		<p><label for="buddypass"><?php _e( 'Passwort erforderlich', 'buddydrive' );?></label>
...
function buddydrive_editor( $editor_id = '' ) {
	...
	?>
	<a href="<?php echo esc_url( $url );?>#TB_inline?inlineId=buddydrive-public-uploader" title="<?php esc_attr_e( 'Datei hinzufügen', 'buddydrive' );?>" id="buddydrive-btn" class="thickbox button">
		<?php echo esc_html_e( 'Datei hinzufügen', 'buddydrive' ); ?>
	</a>
