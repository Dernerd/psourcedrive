...
class BuddyDrive_Group extends BP_Group_Extension {
	...
	public function edit_screen( $group_id = null ) {
		...
		<h4><?php echo esc_attr( $this->name ) ?> <?php _e( 'Einstellungen', 'buddydrive' );?></h4>

		<fieldset>
			<legend class="screen-reader-text"><?php echo esc_attr( $this->name ) ?> <?php _e( 'Einstellungen', 'buddydrive' );?></legend>
			<p><?php _e( 'Mitgliedern dieser Gruppe erlauben, ihre Ordner oder Dateien freizugeben.', 'buddydrive' ); ?></p>

			<div class="field-group">
				<div class="checkbox">
					<label><input type="checkbox" name="_group_buddydrive_activate" value="1" <?php checked( $checked )?>> <?php printf( __( '%s aktivieren', 'buddydrive' ), $this->name );?></label>
				</div>
			</div>

			<?php if ( ! is_admin() ) : ?>
				<input type="submit" name="save" value="<?php _e( 'Speichern', 'buddydrive' );?>" />
			<?php endif; ?>
	...
	public function edit_screen_save( $group_id = null ) {
		...
		if ( ! is_admin() ) {
			...
			if ( !$success )
				bp_core_add_message( __( 'Beim Speichern ist ein Fehler aufgetreten. Bitte versuche es erneut', 'buddydrive' ), 'error' );
			else
				bp_core_add_message( __( 'Einstellungen erfolgreich gespeichert', 'buddydrive' ) );
	...
	public function display( $group_id = null ) {
		...
		if ( ( 'public' !== $this->visibility || 'public' !== $current_group->status ) && ! groups_is_user_member( bp_loggedin_user_id(), $current_group->id ) && ! bp_current_user_can( 'bp_moderate' ) ) {
			printf( '<div id="message" class="info"><p>%s</p></div>', esc_html__( 'Du musst Mitglied dieser Gruppe sein, um die Dateien anzuzeigen.', 'buddydrive' ) );
			return;
		}
