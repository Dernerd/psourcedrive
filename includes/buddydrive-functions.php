...
function buddydrive_activation() {
	...
	if ( empty( $directory_pages[ $buddydrive_slug ] ) ) {
		...
		$buddydrive_page_content = __( 'BuddyDrive verwendet diese Seite, um die Downloads Deiner Buddies-Dateien zu verwalten. Bitte lasse sie unverändert. Es wird nicht in Deiner Navigationsleiste angezeigt.', 'buddydrive');
...
function buddydrive_get_upload_error_strings() {
	...
	$upload_errors = array(
		9  => __( 'Nicht genügend Speicherplatz zum Hochladen Deiner Datei vorhanden', 'buddydrive' ),
		10 => sprintf( __('Diese Datei ist zu groß. Dateien müssen kleiner als %s MB sein.', 'buddydrive' ), buddydrive_max_upload_size() ),
		11 => __( 'Du hast Dein Speicherplatzkontingent aufgebraucht. Bitte lösche Dateien vor dem Hochladen.', 'buddydrive' ),
	);
...
function buddydrive_get_stati( $no_filter = false ) {
	$stati = array(
		'buddydrive_public' => array(
			'label'                     => _x( 'Öffentlich', 'file or folder status', 'buddydrive' ),
			'public'                    => true,
		...
		'buddydrive_private' => array(
			'label'                     => _x( 'Privat', 'file or folder status', 'buddydrive' ),
			'private'                   => true,
			'show_in_admin_status_list' => false,
			'show_in_admin_all_list'    => false,
			'buddydrive_settings'       => true,
		...
		'buddydrive_password' => array(
			'label'                     => _x( 'Passwortgeschützt', 'file or folder status', 'buddydrive' ),
			'protected'                 => true,
			'show_in_admin_status_list' => false,
			'show_in_admin_all_list'    => false,
		...
		'buddydrive_friends' => array(
			'label'                     => _x( 'Auf Freunde beschränkt', 'file or folder status', 'buddydrive' ),
			'protected'                 => true,
			'show_in_admin_status_list' => false,
			'show_in_admin_all_list'    => false,
			'buddydrive_settings'       => false,
		...
		'buddydrive_groups' => array(
			'label'                     => _x( 'Auf eine Gruppe beschränkt', 'file or folder status', 'buddydrive' ),
			'protected'                 => true,
			'show_in_admin_status_list' => false,
			'show_in_admin_all_list'    => false,
			'buddydrive_settings'       => false,
		...
		'buddydrive_members' => array(
			'label'                     => _x( 'Auf Mitglieder beschränkt', 'file or folder status', 'buddydrive' ),
			'protected'                 => true,
			'show_in_admin_status_list' => false,
			'show_in_admin_all_list'    => false,
			'buddydrive_settings'       => false,
...
function buddydrive_update_items_status( $per_page = false ) {
	...
	foreach ( $items as $item ) {
		...
		if ( empty( $update_r ) ) {
			error_log( sprintf( 'Die Artikel-ID %s konnte nicht auf den Status %s aktualisiert werden.', $item->post_id, $status ) );
	...
	$routines = array(
		'200' => array(
			array(
				...
				'message'   => _x( 'Status von Dateien und Ordnern - %d Dateien zum Upgrade', 'Feedback-Nachricht des Upgraders', 'buddydrive' ),
				'callback'  => 'buddydrive_update_items_status'
			),
			array(
				'action_id' => 'upgrade_db_version',
				'count'     => 1,
				'message'   => _x( 'Datenbankversion - 1 zu aktualisierendes Element', 'Feedback-Nachricht des Upgraders', 'buddydrive' ),
				'callback'  => ''
			),
		),
	);
