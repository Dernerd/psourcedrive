...
function buddydrive_upload_file() {
	...
	if ( empty( $buddyfile_id ) ) {
		bp_attachments_json_response( false, $is_html4, array(
			'type'    => 'upload_error',
			'message' => __( 'Fehler beim Erstellen der Datei, sorry.', 'buddydrive' ),
		) );
	} else {
...
function buddydrive_fetch_items() {
	if ( 'POST' !== strtoupper( $_SERVER['REQUEST_METHOD'] ) ) {
		wp_send_json_error( array(
			'message' => __( 'Die Aktion wurde nicht korrekt gesendet.', 'buddydrive' ),
		) );
	}

	$not_allowed = array( 'message' => __( 'Du darfst diese Aktion nicht ausführen.', 'buddydrive' ) );
	...
	if ( ! empty( $query_args['buddydrive_parent'] ) ) {
		...
		$metas['post_parent_infos'] = sprintf(
			_x( 'Dateien, die diesem Ordner hinzugefügt werden, haben den folgenden Datenschutz: %s', 'Datenschutzinformationen für Elternordner', 'buddydrive' ),
		...
		if ( 'buddydrive_groups' === $post_parent->post_status ) {
			...
			if ( ! empty( $object->id ) ) {
				...
				$metas['post_parent_infos'] = sprintf(
					_x( 'Dateien, die diesem Ordner hinzugefügt wurden, sind für Mitglieder dieser Gruppe zugänglich: %s', 'Datenschutzinformationen für Elternordner', 'buddydrive' ),
					sprintf(
			...
			if ( ! empty( $members ) ) {
				...
				if ( ! empty( $avatars ) ) {
					$metas['post_parent_infos'] = sprintf(
						_x( 'Dateien, die diesem Ordner hinzugefügt werden, sind für die folgenden Mitglieder zugänglich: %s', 'Datenschutzinformationen für Elternordner', 'buddydrive' ),
						join( ' ', $avatars )
					);
				}
			}
	...
	if ( empty( $items ) ) {
		$metas['no_items_found'] = __( 'Keine Dateien gefunden.', 'buddydrive' );
...
function buddydrive_item_update() {
	...
	if ( empty( $_POST['buddydrive_nonce'] ) || ! wp_verify_nonce( $_POST['buddydrive_nonce'], 'buddydrive_update_item' ) ) {
		wp_send_json_error( array(
			'message' => __( 'Du darfst diese Aktion nicht ausführen.', 'buddydrive' ),
		) );
	}
	...
	$error = array( 'message' => __( 'Unbekannte Datei.', 'buddydrive' ) );
	...
	if ( ! buddydrive_update_item( $r, $item ) ) {
		wp_send_json_error( array(
			'message' => __( 'Etwas ist schief gelaufen. Bitte versuche es später noch einmal.', 'buddydrive' ),
		) );
	}

	wp_send_json_success( array(
		'message' => __( 'Datei erfolgreich aktualisiert.', 'buddydrive' ),
	) );
}
...
function buddydrive_get_bpobjects() {
	...
	if ( empty( $objects ) ) {
		wp_send_json_error( array( 'error' => __( 'Es wurden keine Dateien gefunden.', 'buddydrive' ) ) );
	} else {
...
function buddydrive_bulk_edit_items() {
	...
	$error = array( 'error' => __( 'Ein unerwarteter Fehler ist aufgetreten.', 'buddydrive' ) );
	...
	if ( empty( $r['items'] ) ) {
		wp_send_json_error( array( 'error' => __( 'Es wurden keine Dateien gesendet.', 'buddydrive' ) ) );
	}
...
function buddydrive_get_stats() {
	...
	if ( ! empty( $stats) ) {
		...
		foreach ( $stats as $stat ) {
			...
			$response['detail'][] = array(
				...
				'stat'  => sprintf( _n( '%d Datei', '%d Dateien', $stat->num, 'buddydrive' ), number_format_i18n( $stat->num ) ),
			);
	...
	if ( ! empty( $total ) ) {
		$response['total'] = sprintf( _n( '(%d Datei)', '(%d Dateien)', $total, 'buddydrive' ), number_format_i18n( $total ) );
	}
