...
function buddydrive_localize_ui() {
	...
	foreach ( (array) buddydrive_get_sharing_options() as $option => $label ) {
		...
		if ( 'groups' === $option || 'members' === $option ) {
			...
			if ( 'members' === $option ) {
				$privacy_filters[ $option ]['autocomplete_placeholder'] = __( 'Beginne mit der Eingabe eines Mitgliedsnamens', 'buddydrive' );
			} else {
				$privacy_filters[ $option ]['autocomplete_placeholder'] = __( 'Beginne mit der Eingabe eines Gruppennamens', 'buddydrive' );
			}
		}
	...
	$privacy_filters[ 'folder' ] = array(
		'text'                     => __( 'Zu einem bestehenden Ordner hinzufügen', 'buddydrive' ),
		'priority'                 => $priority,
		'allow_multiple'           => false,
		'autocomplete_placeholder' => __( 'Beginne mit der Eingabe eines Ordnernamens', 'buddydrive' )
	);
	...
	$l10n_array['settings'] =  array(
		...
		'loop_filters'     => array(
			'modified'     => array(
				'text'     => __( 'Letzte Änderung', 'buddydrive' ),
				'priority' => 5,
			),
			'title'        => array(
	...
	$l10n_array['strings'] = array(
		'allCrumb'           => __( 'Alle', 'buddydrive' ),
		'loadMore'           => __( 'Mehr laden', 'buddydrive' ),
		'privacyFilterLabel' => __( 'Wähle Deine Datenschutzeinstellungen aus.', 'buddydrive' ),
		'passwordInputLabel' => __( 'Definiere dein Passwort.', 'buddydrive' ),
		'loopFilterLabel'    => __( 'Artikel sortieren nach:', 'buddydrive' ),
		'editErrors'         => array(
			'title'          => __( 'Bitte stelle sicher, dass Du einen Namen für Deine Datei oder Deinen Ordner angibst.', 'buddydrive' ),
			'groups'         => __( 'Bitte stelle sicher, dass Du eine Gruppe über das Feld für die automatische Vervollständigung auswählst.', 'buddydrive' ),
			'members'        => __( 'Bitte stelle sicher, dass Du ein Mitglied über das Feld für die automatische Vervollständigung auswählst.', 'buddydrive' ),
			'folder'         => __( 'Bitte stelle sicher, dass Du einen Ordner mit dem Autocomplete-Feld auswählst.', 'buddydrive' ),
			'password'       => __( 'Bitte stelle sicher, dass Du ein Passwort für Deine Datei oder Deinen Ordner angibst.', 'buddydrive' ),
		),
		'saveEdits'          => __( 'Deine Änderungen werden gespeichert, bitte warten.', 'buddydrive' ),
	);
	...
	if ( buddydrive_current_user_can( 'buddydrive_upload' ) ) {
		$l10n_array['settings']['manage_toolbar'] = array(
			'new_file' => array(
				'id'       => 'new_file',
				'text'     => __( 'Neue Datei', 'buddydrive' ),
				'dashicon' => 'welcome-add-page',
			),
			'new_folder' => array(
				'id'       => 'new_folder',
				'text'     => __( 'Neuer Ordner', 'buddydrive' ),
				'dashicon' => 'portfolio',
			),
			'bulk' => array(
				'id'       => 'bulk',
				'text'     => __( 'Massenaktionen', 'buddydrive' ),
				'dashicon' => 'forms',
			),
		);
		...
		$l10n_array['settings']['bulk_actions'] = array(
			'delete' => array(
				'id'   => 'delete',
				'text' => __( 'Ausgewählte Elemente löschen', 'buddydrive' ),
			),
			'remove' => array(
				'id'   => 'remove',
				'text' => __( 'Ausgewählte Dateien aus Ordner entfernen', 'buddydrive' ),
			),
		);
		...
		if ( bp_is_group() ) {
			$l10n_array['settings']['bulk_actions'] = array( 'remove' => array(
					'id'   => 'group_remove',
					'text' => __( 'Ausgewählte Elemente aus der Gruppe entfernen', 'buddydrive' ),
				),
			);
		}

		$l10n_array['strings']['new_folder_name']   = __( 'Name Deines Ordners', 'buddydrive' );
		$l10n_array['strings']['new_folder_button'] = __( 'Ordner erstellen', 'buddydrive' );
	}
...
function buddydrive_user_buddydrive_url( $linkonly = false ) {
	...
	if( ! empty($linkonly ) ) {
		...
		$output = apply_filters( 'buddydrive_user_buddydrive_url', '<a href="'. esc_url( $url ) .'" title="' . esc_attr__( 'Wähle oder füge eine Datei aus meinem Profil hinzu', 'buddydrive' ) .'" class="buddydrive-profile"><i class="icon bd-icon-newfile"></i> ' . esc_html__( 'Dateien verwalten', 'buddydrive' ) .'</a>' );
		echo $output;
	}
...
function buddydrive_check_download( $file = null, $user_id = 0 ) {
	if ( empty( $file ) || empty( $file->check_for ) ) {
		return new WP_Error( 'empty_request', __( 'Entschuldigung, unbekannte Datei oder unbekannter Benutzer.', 'buddydrive' ), 401 );
	}
	...
	switch ( $file->check_for ) {
		...
		case 'password' :
			if ( ! isset( $file->pass_submitted ) ) {
				$can_download = new WP_Error( 'empty_password', __( 'Diese Datei ist passwortgeschützt', 'buddydrive' ), 403 );

			} else {
				if ( $file->password === $file->pass_submitted ) {
					$can_download = true;
				} else {
					$can_download = new WP_Error( 'wrong_password', __( 'Falsches Passwort', 'buddydrive' ), 401 );
				}
			}
		break;
		...
		case 'friends' :
			if ( bp_is_active( 'friends' ) && friends_check_friendship( $file->user_id, $user_id ) ) {
				...
				$can_download = new WP_Error( 'not_friend', __( 'Du musst ein Freund dieses Mitglieds sein, um die Datei herunterzuladen', 'buddydrive' ), buddydrive_get_user_buddydrive_url( $file->user_id ) );
			}
		break;
		...
		case 'members' :
			if ( is_array( $file->members ) && in_array( $user_id, $file->members ) ) {
				...
				$can_download = new WP_Error( 'not_in_members', __( 'Du musst vom Eigentümer der Datei zu den zugelassenen Mitgliedern hinzugefügt werden, um sie herunterladen zu können', 'buddydrive' ), buddydrive_get_user_buddydrive_url( $file->user_id ) );
			}
		break;

		case 'groups' :
			if ( ! bp_is_active( 'groups' ) ) {
				$can_download = new WP_Error( 'groups_inactive', __( 'Gruppenkomponente ist deaktiviert, bitte kontaktiere den Administrator.', 'buddydrive' ), buddydrive_get_root_url() );
			} else {
				// Validate & get groups
				...
				if ( ! empty( $groups['groups'] ) ) {
					...
					if ( ! $can_download ) {
						...
						$can_download = new WP_Error( 'not_in_groups', __( 'Du musst Mitglied der Gruppe sein, um die Datei herunterzuladen', 'buddydrive' ), $redirect );
					}
				} else {
					$can_download = new WP_Error( 'unfound_groups', __( 'Die Gruppen, an die die Datei angehängt ist, stimmen mit keinen vorhandenen Gruppen überein.', 'buddydrive' ), buddydrive_get_root_url() );
				}
			}
		break;
		...
		case 'private' :
		default        :
			$can_download = new WP_Error( 'unauthorized_download', __( 'Du darfst diese Datei leider nicht herunterladen.', 'buddydrive' ), 401 );
