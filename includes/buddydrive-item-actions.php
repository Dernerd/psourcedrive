...
function buddydrive_file_downloader() {
	...
	if ( ! bp_displayed_user_id() && bp_is_current_component( 'buddydrive' ) && 'file' == bp_current_action() ) {
		...
		if ( empty( $buddydrive_file ) ) {
			bp_core_add_message( __( 'Hoppla, wir konnten Deine Datei nicht finden.', 'buddydrive' ), 'error' );
		...
		if ( is_wp_error( $can_download ) ) {
			if ( 401 === $can_download->get_error_data() ) {
				wp_die( __( 'Du darfst diese Datei nicht herunterladen.', 'buddydrive' ) , __( 'Unautorisierter Zugriff', 'buddydrive' ), 401 );
		...
		if ( empty( $buddyfolder ) ) {
			bp_core_add_message( __( 'Hoppla, wir konnten Deinen Ordner nicht finden.', 'buddydrive' ), 'error' );
			bp_core_redirect( buddydrive_get_root_url() );
		}
