	...
	<# } else { #>
		...
		<div class="buddydrive-content">
			...
			<div class="buddydrive-actions">
				...
				<# if ( data.can_edit ) { #>
					<a class="buddydrive-edit" href="#edit/{{data.id}}"><span class="screen-reader-text bp-screen-reader-text"><?php esc_html_e( 'Bearbeiten', 'buddydrive' );?></span></a>
				<# } #>

				<?php do_action( 'buddydrive_js_template_file_actions' ); ?>

				<# if ( data.can_share ) { #>
					<a class="buddydrive-share" href="#"><span class="screen-reader-text bp-screen-reader-text"><?php esc_html_e( 'Teilen', 'buddydrive' );?></span></a>
				<# } #>
			</div>
		</div>
		<# if ( data.can_share ) { #>
			<div class="buddydrive-share-dialog buddydrive-hide">
				<div class="buddydrive-share-content">
					<div class="buddydrive-share-input">
						<p class="description"><?php esc_html_e( 'Verwende den folgenden Link, um einen Einbettungscode auf dieser Webseite zu teilen.', 'buddydrive' ); ?></p>
						<input type="text" class="buddydrive-share-url" value="{{data.link}}" tabindex="0" readonly>
					</div>
				</div>
				<button type="button" class="buddydrive-share-dialog-close" aria-label="<?php esc_attr_e( 'Freigabedialog schließen', 'buddydrive' ); ?>">
...
<script type="text/html" id="tmpl-buddydrive-edit-header">
	...
	<h2 class="buddydrive-title"><?php esc_html_e( 'Bearbeitung:', 'buddydrive' ); ?> <a href="{{data.link}}">{{data.title}}</a></h2>
</script>
...
<script type="text/html" id="tmpl-buddydrive-object">
	...
	<# if ( data.selected ) { #>
		<a href="#" class="buddydrive-object-remove" data-item_id="{{data.id}}">
			<span class="screen-reader-text bp-screen-reader-text"><?php esc_html_e( 'Entfernen', 'buddydrive' ); ?></span>
		</a>
	<# } #>
</script>
...
<script type="text/html" id="tmpl-buddydrive-edit-details">
	<# if ( 'undefined' !== typeof data.user_avatar ) { #>
		<label for="buddydrive-owner-edit"><?php esc_html_e( 'Besitzer', 'buddydrive' ); ?></label>
		<a class="buddydrive-owner" href="{{data.user_link}}" data-user-id="{{data.user_id}}">{{{data.user_avatar}}}</a>
	...
	<label for="buddydrive-content-edit"><?php esc_html_e( 'Beschreibung', 'buddydrive' ); ?></label>
	...
	<# if ( 'undefined' !== typeof data.post_parent_title ) { #>
		<div id="buddydrive-item-folder">
			<label for="buddydrive-folder-edit"><?php esc_html_e( 'Übergeordneter Ordner', 'buddydrive' ); ?></label>
			...
			<a id="buddydrive-remove-parent" href="#" title="<?php esc_attr_e( 'Aus diesem Ordner entfernen', 'buddydrive' ); ?>">
				<span class="screen-reader-text bp-screen-reader-text">
					<?php esc_html_e( 'Aus diesem Ordner entfernen', 'buddydrive' );?>
				</span>
			</a>
			...
			<p class="descrition"><?php esc_html_e( 'Die Privatsphäre dieser Datei hängt von seinem übergeordneten Ordner ab.', 'buddydrive' ); ?></p>
		</div>
	<# } #>
	...
	<div class="submit">
		<input type="reset" value="<?php esc_attr_e( 'Abbrechen', 'buddydrive' ); ?>" class="button button-secondary"><input type="submit" value="<?php esc_attr_e( 'Bearbeiten', 'buddydrive' ); ?>"  class="button button-primary">
	</div>
</script>
...
<script type="text/html" id="tmpl-buddydrive-stats">
	...
	<# } else { #>
		...
		<p><?php esc_html_e( 'Verwendeter Speicherplatz:', 'buddydrive' ); ?> <strong>{{data.used}}%</strong> {{data.total}}</p>
